...
from email_analyzer.attachment_scanner import defender_product_status,scan_attachment,scan_with_clamav
...
class AttachmentScannerTests(unittest.TestCase):
    ...
    def scan(self,*args,**kwargs):
        kwargs.setdefault('clamav_executable',self.root/'missing-clamscan.exe')
        return scan_attachment(*args,**kwargs)
    def test_clean_scan_is_non_destructive_custom_scan(self):
        runner=Mock(return_value=self.result(0,b'no malware found'))
        result=self.scan(self.file,executable=self.tool,runner=runner)
    ...
    def test_detection_and_ambiguous_exit_are_distinct(self):
        detected=self.scan(self.file,executable=self.tool,runner=Mock(return_value=self.result(2,b'detected 1 threat')))
        alert=self.scan(self.file,executable=self.tool,runner=Mock(return_value=self.result(2,b'scanning error')))
    ...
    def test_disabled_defender_is_reported_without_false_alert(self):
        ...
        result=self.scan(self.file,executable=self.tool,runner=Mock(return_value=self.result(2,output)))
    ...
    def test_clamav_exit_codes_are_distinct(self):
        clean=scan_with_clamav(self.file,executable=self.tool,runner=Mock(return_value=self.result(0,b'')))
        threat=scan_with_clamav(self.file,executable=self.tool,runner=Mock(return_value=self.result(1,b'sample.bin: Test.Signature FOUND\n')))
        failed=scan_with_clamav(self.file,executable=self.tool,runner=Mock(return_value=self.result(2,b'database missing')))
        self.assertEqual(clean['status'],'clean')
        self.assertEqual(threat['status'],'threat_detected')
        self.assertEqual(threat['threat_name'],'Test.Signature')
        self.assertEqual(failed['status'],'error')
    def test_clamav_threat_overrides_disabled_defender(self):
        clam_runner=Mock(return_value=self.result(1,b'sample.bin: Test.Signature FOUND\n'))
        result=scan_attachment(self.file,executable=self.root/'none.exe',
                               clamav_executable=self.tool,clamav_runner=clam_runner)
        self.assertEqual(result['status'],'threat_detected')
    ...
    def test_missing_scanner_and_timeout_are_not_clean(self):
        ...
        timeout=self.scan(self.file,executable=self.tool,runner=runner,timeout=1)
    ...
    def test_disguised_executable_and_archive_member(self):
        ...
        result=self.scan(disguised,executable=self.root/'none.exe')
        ...
        result=self.scan(archive,executable=self.root/'none.exe')
    ...
    def test_script_download_execution_chain(self):
        ...
        result=self.scan(script,executable=self.root/'none.exe')
        self.assertEqual(result['status'],'suspicious_structure')
