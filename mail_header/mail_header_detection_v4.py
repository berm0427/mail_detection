...
class EmailHeaderAnalyzer:
    ...
    def analyze_header_chain(self, msg):
        ...
        # 그렇지 않은 경우 기술적 송신 도메인 우선
            ...
            actual_sender_domain = technical_sender_domain or from_domain
    ...
    def _resolve_dns(self, domain, record_type):
        records = self.analysis_result['details'].setdefault('dns_queries', [])
        try:
            answer = dns.resolver.resolve(domain, record_type, lifetime=4)
            records.append({'domain': domain, 'type': record_type, 'status': 'ok', 'count': len(answer)})
            return answer
        except Exception as exc:
            status = ('nxdomain' if isinstance(exc, dns.resolver.NXDOMAIN) else
                      'no_record' if isinstance(exc, dns.resolver.NoAnswer) else
                      'timeout' if isinstance(exc, dns.exception.Timeout) else 'error')
            records.append({'domain': domain, 'type': record_type, 'status': status})
            raise
    ...
    def check_dns_records(self, domain):
        ...
        try:
            ...
            # A 레코드 검사
            try:
                a_records = self._resolve_dns(domain, 'A')
            ...
            # MX 레코드 검사
            try:
                mx_records = self._resolve_dns(domain, 'MX')
    ...
    def check_spf_record(self, domain):
        """도메인의 SPF 레코드 확인"""
        if not hasattr(self, '_spf_seen'):
            self._spf_seen = set()
        if domain in self._spf_seen or len(self._spf_seen) >= 10:
            return False
        self._spf_seen.add(domain)
        try:
            answers = self._resolve_dns(domain, 'TXT')
    ...
    def analyze_email(self, raw_email):
        ...
        try:
            ...
            # Reserved test-only suffixes cannot establish public sender identity.
            reserved = sender_domain.rstrip('.').lower().split('.')[-1] in {'example', 'invalid', 'test', 'localhost'}
            if reserved:
                self.analysis_result['details']['dns_queries'] = [{'domain': sender_domain, 'type': 'ALL', 'status': 'reserved_test_domain'}]
                self.analysis_result['reasons'].append('예약된 테스트 도메인: 공개 DNS/WHOIS 인증 근거로 사용하지 않음')
            else:
    ...
    def _build_final_result(self):
        ...
        return {
            ...
            'sender_domain': self.analysis_result.get('sender_domain', ''),
            'technical_sender_domain': self.analysis_result.get('technical_sender_domain'),
            'from_domain': self.analysis_result.get('from_domain', ''),
